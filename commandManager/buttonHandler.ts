...
import { getResponsePrelude, getFailurePrelude } from "../functions/messagePreludes.js";

// Store pending deletions with user ID and timestamp
const pendingDeletions = new Map<string, { queryType: string; queryId: string; timestamp: number }>();
...
async function handleConfirmDelete(interaction: ButtonInteraction) {
  ...
  try {
    ...
    if (error?.code === 'P2025') { // Prisma record not found error
      ...
      await interaction.editReply({
        content: `${getFailurePrelude()} Failed to delete query: ${error?.message || 'Unknown error'}`,
        components: []
      });
    }
  }
