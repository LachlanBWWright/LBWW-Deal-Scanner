...
echo "Starting Go backend and frontend. Press Ctrl+C to stop both."
echo "Using API_PORT=$SERVER_PORT and frontend port $FRONTEND_PORT"
echo "Testing API enabled: $ENABLE_TESTING_VALUE"

run_with_prefix "goserver" "goserver" env API_PORT="$SERVER_PORT" API_SECRET="$API_SECRET_VALUE" ENABLE_TESTING_API="$ENABLE_TESTING_VALUE" /usr/local/go/bin/go run cmd/dealscanner/main.go &
SERVER_PID=$!
