...
export async function getGumtreeValues(page: Page, item: Gumtree) {
  ...
  let result = page.locator(
    "a[class='user-ad-row-new-design link link--base-color-inherit link--hover-color-none link--no-underline']",
  ).first(); //#react-root > div > div.page > div > div.search-results-page__content > main > section > div

  if (!(await result.count())) throw new Error("No ressult");
  ...
  const foundImg = await result.locator("img").evaluate((img: HTMLImageElement) => img.src);
